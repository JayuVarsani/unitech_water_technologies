<div x-data="jobcard">
    ...
    <div class="card mb-5 mb-xl-10">
        ...
        <div class="me-2 mb-6" style="margin-left:90%" x-data="jobcard">
            <button class="btn btn-sm btn-flex btn-warning fw-bold btn-sm" x-ref="filterDiv" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end">
        ...
        <form class="form" method="post" wire:submit.prevent="save" enctype="multipart/form-data">
            @csrf @if($jobcard->isNotEmpty() || $jobcardcom->isNotEmpty())
                    ...
                    <div class="col-lg-8">
                        <div class="card card-stretch card-bordered mb-2">
                            ...
                            <div class="card-footer">
                                <button type="button" class="btn btn-primary me-2" style="margin-left:26%" data-bs-toggle="modal" data-bs-target="#wastageModal" x-on:click.prevent="openWastageModal(selectedJobDetails.id)">
                                    Wastage
                                </button>
                    ...
                    <div class="modal-body">
                            ...
                            <div x-show="showMaterialList" x-transition>
                                <div class="mb-3">
                                    ...
                                    <div class="form-check mb-2">
                                        <input class="form-check-input" type="checkbox" id="material_{{ $material->id }}" value="{{ $material->id }}" wire:model="selectedMaterials" @click="showDimensions[{{ $material->id }}] = !showDimensions[{{ $material->id }}]">
                                    ...
                                    <div x-show="showDimensions[{{ $material->id }}]" x-transition>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <input type="number" class="form-control" id="height_{{ $material->id }}" wire:model="dimensions.{{ $material->id }}.height" placeholder="Height">
                                            </div>
                                            <div class="col-md-6">
                                                <input type="number" class="form-control" id="width_{{ $material->id }}" wire:model="dimensions.{{ $material->id }}.width" placeholder="Width">
                                            </div>
                                        </div>
                                    </div>
                                    @endforeach
...
<script>
  ...
  Alpine.data('jobcard', () => ({
    ...
    modalElement.addEventListener('hidden.bs.modal', () => {
        @this.reset(); // Reset Livewire properties when modal is closed
    });
}
    }));
