                ...
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input type="text" wire:model.live.debounce.800ms="query.search"
                            ...
                            placeholder="{{__('app.panel.search_name', ['name'=>__('company.staff-management.staff-role')])}}">
                    </div>
                </div>
            @endif
            @if($canCreate)
                <div class="card-toolbar">
                    <div class="d-flex justify-content-end">
                        <a href="{{ route('company.staff-management.staff-role.create') }}" class="btn btn-primary btn-sm">
                            <i class="ki-duotone ki-plus fs-2"></i> {{__('app.panel.create_name', ['name' =>
                            __('company.staff-management.staff-role')])}}
                        </a>
                    </div>
                </div>
            @endif
                          ...
                          <td class="text-start">{{ $items->total() - (($items->currentPage() - 1) * $items->perPage()) - ($loop->iteration - 1) }}</td>
                            ...
                            <td>
                                <x-panel::table.action.main>
                                    @if($canEdit)
                                        <x-panel::table.action.edit :route="route('company.staff-management.staff-role.edit',$item->id)" module="{{__('company.staff-management.staff-role')}}" />
                                    @endif
                                    @if($canDelete)
