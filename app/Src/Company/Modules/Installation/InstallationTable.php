...
class InstallationTable extends BaseTable
{
    use ExceptionTrait;
    ...
    {
        return view('company::Installation.views.table', [
            'items' => $this->dataSource(),
        ])->layout('panel::layout.app', ['title' => 'Installations']);
    }
    ...
    public function delete(Installation $installation): void
    {
        ...
        $installation->delete();
        flashAlert(__('app.panel.delete', ['name' => 'Installation']), 'success');
    }

    protected function dataSource(): LengthAwarePaginator
    {
        return Installation::select([
            'id',
            'installation_date',
        ])
            ->withCount(['installationParameters', 'installationTreatmentSchemes'])
            ->where('company_id', Auth::user()->company_id)
            ->when($this->query->search, function (Builder $query) {
                $query->where(function (Builder $query) {
                    $search = $this->query->search;

                    return $query->whereAny(['id'], 'like', "%{$search}%")
                        ->orWhereDate('installation_date', 'like', "%{$search}%");
                });
            })
            ->latest('id')
