                ...
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input type="text" wire:model.live.debounce.800ms="query.search"
                            ...
                            placeholder="{{__('app.panel.search_name', ['name'=>'Treatment Scheme'])}}">
                    </div>
                </div>
            @endif
            @if($canCreate)
                <div class="card-toolbar">
                    <div class="d-flex justify-content-end">
                        <a href="{{ route('company.treatment-scheme.create') }}" class="btn btn-primary btn-sm">
                            <i class="ki-duotone ki-plus fs-2"></i> {{__('app.panel.create_name', ['name' => 'Treatment Scheme'])}}
                        </a>
                    </div>
                </div>
            @endif
                        ...
                        <tr wire:key="{{ $item->id }}">
                            ...
                            <td>
                                <x-panel::table.action.main>
                                    @if($canEdit)
                                        <x-panel::table.action.edit :route="route('company.treatment-scheme.edit',$item->id)" module="Treatment Scheme" />
                                    @endif
                                    @if($canDelete)
