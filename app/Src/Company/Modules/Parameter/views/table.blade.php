                ...
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input type="text" wire:model.live.debounce.800ms="query.search"
                            ...
                            placeholder="{{__('app.panel.search_name', ['name'=>'Parameter'])}}">
                    </div>
                </div>
            @endif
            @if($canCreate)
                <div class="card-toolbar">
                    <div class="d-flex justify-content-end">
                        <a href="{{ route('company.parameter.create') }}" class="btn btn-primary btn-sm">
                            <i class="ki-duotone ki-plus fs-2"></i> {{__('app.panel.create_name', ['name' => 'Parameter'])}}
                        </a>
                    </div>
                </div>
            @endif
            ...
            <div class="card-body pt-0">
                <x-panel::table.main :items="$items">
                    <x-panel::table.head>
                        ...
                        <th class="min-w-125px text-start text-black">Unit</th>
                        ...
                        <tr wire:key="{{ $item->id }}">
                            ...
                            <td>
                                <x-panel::table.action.main>
                                    @if($canEdit)
                                        <x-panel::table.action.edit :route="route('company.parameter.edit',$item->id)" module="Parameter" />
                                    @endif
                                    @if($canDelete)
