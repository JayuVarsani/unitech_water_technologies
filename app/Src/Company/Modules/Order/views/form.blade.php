        ...
        <form class="form" method="post" wire:submit="save" enctype="multipart/form-data">
                ...
                <div class="row mb-6 justify-content-between">
                    <div class="col-md-2">
                        <label class="col-lg-12 col-form-label fw-semibold fs-6"
                            for="order_id">{{ __('company.order_id') }}</label>
                            ...
                            <select class="form-select form-select-lg add-select2 form-control border-on-focus"
                                ...
                                @foreach ($customer as $single_customer)
                                    <option wire:key="customer-{{ $single_customer->id }}"
                                        value="{{ $single_customer->id }}">{{ $single_customer->name }} ({{ $single_customer->contact_number }})</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-md-2">
                        <label class="col-lg-12 col-form-label fw-semibold fs-6 required"
                            for="order_status">{{ __('company.input.status') }}
                        </label>
                        <div wire:ignore class="col-lg-12 fv-row">
                            <select class="form-select form-select-lg form-control" wire:model.live="form.order_status"
                                id="order_status">
                                <option value="">
                                    {{ __('company.placeholder.select', ['name' => __('company.input.status')]) }}
                                </option>
                                @foreach (\App\Utility\Enums\OrderStatusEnum::cases() as $status)
                                    <option wire:key="order-status-{{ $status->value }}" value="{{ $status->value }}">
                                        {!! $status->getName() !!}</option>
                                @endforeach
                            </select>
                        </div>
                        <x-panel::error name="form.order_status" />
                    </div>
                </div>
                <hr class="text-muted">
                                    ...
                                    <div class="col-12 col-sm-6 col-lg-2">
                                        ...
                                        <div class="col-lg-12 fv-row">
                                            <input type="number" id="width" name="width" min="0"
                                                {{-- @readonly(!isset($this->form->product_id))  --}}
                                ...
                                <x-panel::table.body :items="$selectedJobs">
                                    @foreach ($selectedJobs as $index => $job)
                                        <tr wire:key="job-{{ $index }}">
                                            ...
                                            <td>{{ $job['status'] ?? '-' }} <x-panel::error
                                                    name="selectedJobs.{{ $index }}.status" />
                                            </td>
                                            ...
                                            {{-- <td>{{ $job['transportation_charge'] ?? '-' }} <x-panel::error
                                                        name="selectedJobs.{{ $index }}.transportation_charge" />
                                                </td> --}}
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-center action-div">
                                                    <span title="{{ $job['narration'] }}">
                                                        ...
                                                        @else
                                                            <span style="cursor: not-allowed"><i
                                                    ...
                                                    @if (
                                                        ...
                                                        <a style="cursor: pointer" data-toggle="tooltip"
                                                            ...
                                                            <i class="fa-solid fa-pencil icon text-primary"
                                                                wire:loading.remove wire:target="editJob('{{ $index }}')"></i>
                                                            <x-panel::inline-loader target="editJob('{{ $index }}')" />
                                                        </a>
                                                    @else
                                                    ...
                                                    @endif
                                                    {{-- show delete only if job is new or in design status --}}
                                                    {{-- @if (!isset($job['job_no']) || (isset($job['status']) && $job['status'] == \App\Utility\Enums\OrderJobStatusEnum::Design->value))
                                                        <a style="cursor: pointer" data-toggle="tooltip"
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center"><span
                                                class="text-muted fw-bold fs-7 text-uppercase gs-0">{{ __('company.input.total') }}</span>
                                            {{ $selectedJobs->sum('pesting_charge') ?? '-' }}</td>
                                        <td class="text-center"><span
                                                class="text-muted fw-bold fs-7 text-uppercase gs-0">{{ __('company.input.total') }}</span>
                                            {{ $selectedJobs->sum('fitting_charge') ?? '-' }}</td>
                                        <td colspan="3"></td>
                                    </tr>
                                </x-panel::table.body>
                                <tr>
                                ...
                                <tr>
                                    <td colspan="10" class="align-top">
                                        <div class="d-flex flex-column gap-4">
                                            ...
                                            @if ($this->form->order_status == \App\Utility\Enums\OrderStatusEnum::Cancelled->value)
                                                <div class="d-flex flex-column gap-2">
                                                    <label class="col-form-label fw-semibold fs-6 text-start p-0"
                                        ...
                                        <div class="card p-0 h-100">
                                            <div class="card-body p-3 border rounded">
                                                ...
                                                <div class="row mb-2 align-items-center">
                                                    ...
                                                    <div class="col-md-6">
                                                        <input type="number" id="job_total_amount" step="1"
                                                            min="0" step="1" {{-- wire:model.blur="form.job_total_amount" --}}
                                                            readonly value="{{ $form->job_total_amount ?? 0 }}"
                                                ...
                                                <div>
                                                    <div class="row mb-2 align-items-center">
                                                        <label for="order_pesting_charge"
                                                            class="col-md-6 col-form-label text-md-end"><strong>{{ __('company.input.pesting_charge') }}
                                                            </strong></label>
                                                        <div class="col-md-6">
                                                            <input type="number" id="order_pesting_charge"
                                                                min="0" step="1"
                                                                wire:model.blur="form.order_pesting_charge"
                                                                class="form-control form-control-sm form-control-solid">
                                                            <x-panel::error name="form.order_pesting_charge" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div>
                                                    <div class="row mb-2 align-items-center">
                                                        <label for="order_fitting_charge"
                                                            class="col-md-6 col-form-label text-md-end"><strong>{{ __('company.input.fitting_charge') }}</strong></label>
                                                        <div class="col-md-6">
                                                            <input type="number" id="order_fitting_charge"
                                                                min="0" step="1"
                                                                wire:model.blur="form.order_fitting_charge"
                                                                class="form-control form-control-sm form-control-solid">
                                                            <x-panel::error name="form.order_fitting_charge" />
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div>
                                                    <div class="row mb-2 align-items-center">
                                                        <label for="order_transportation"
                                                            class="col-md-6 col-form-label text-md-end "><strong>{{ __('company.input.transportation_charge') }}
                                                            </strong></label>
                                                        <div class="col-md-6">
                                                            <input type="number" id="order_transportation"
                                                                min="0" step="1"
                                                                wire:model.blur="form.order_transportation"
                                                                class="form-control form-control-sm form-control-solid">
                                                            <x-panel::error name="form.order_transportation" />
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row mb-2 align-items-center">
                                                    ...
                                                    <div class="col-md-6">
                                                        <input type="number" id="discount" step="1"
                                                            min="0" step="1"
                                                            wire:model.blur="form.discount"
                                                            class="form-control form-control-sm form-control-solid"
            ...
            <div class="card-footer d-flex justify-content-end py-6 px-9">
                ...
                <button type="button" wire:click="save" class="btn btn-primary"
                    id="submitOrderButton">{{ __('app.panel.submit') }}</button>
            </div>
        </form>
    </div>
</div>
