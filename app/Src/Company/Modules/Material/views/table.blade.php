                ...
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input type="text" wire:model.live.debounce.800ms="query.search"
                            ...
                            placeholder="{{__('app.panel.search_name', ['name'=>__('company.material')])}}">
                    </div>
                </div>
            @endif
            @if($canCreate)
                <div class="card-toolbar">
                    <div class="d-flex justify-content-end">
                        <a href="{{route('company.material.create')}}" class="btn btn-primary btn-sm">
            ...
            <div class="card-body pt-0">
                <x-panel::table.main :items="$items">
                    <x-panel::table.head>
                        ...
                        <th class="min-w-125px text-start text-black">{{ __('company.input.material_name') }}</th>
                        <!-- <th class="min-w-125px">{{ __('company.input.sku') }}</th> -->
                        
                        <th class="min-w-125px text-start text-black">{{ __('company.input.stock') }}</th>
                        <!-- <th class="min-w-125px">{{ __('company.input.note') }}</th>
                         -->
                    ...
                    <x-panel::table.body :items="$items">
                        ...
                        <tr wire:key="{{ $item->id }}">
                            ...
                            <td class="text-start">{{ $item->available_stock }}   {{ $item->unit_name }}</td> 
                            ...
                            @if($canEdit || $canDelete)
                                <td>
                                    <x-panel::table.action.main>
                                        @if($canEdit)
                                            <x-panel::table.action.edit :route="route('company.material.edit',$item->id)" module="{{__('company.material')}}" />
                                        @endif
                                        {{-- @if($canDelete)
