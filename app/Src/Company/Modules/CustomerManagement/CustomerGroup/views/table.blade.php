                ...
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input type="text" wire:model.live.debounce.800ms="query.search"
                            ...
                            placeholder="{{__('app.panel.search_name', ['name'=>__('company.customer-group')])}}">
                    </div>
                </div>
            @endif
            @if($canCreate)
                <div class="card-toolbar">
                    <div class="d-flex justify-content-end">
                        <a href="{{route('company.customer-management.customer-group.create')}}" class="btn btn-primary btn-sm">
                            <i class="ki-duotone ki-plus fs-2"></i> {{__('app.panel.create_name', ['name' =>
                            __('company.customer-group')])}}
                        </a>
                    </div>
                </div>
            @endif
                            ...
                            @if($canEdit || $canDelete)
                                <td>
                                    <x-panel::table.action.main>
                                        @if($canEdit)
                                            <x-panel::table.action.edit :route="route('company.customer-management.customer-group.edit',$item->id)" module="{{__('company.customer-group')}}" />
                                        @endif
                                        @if($canDelete)
