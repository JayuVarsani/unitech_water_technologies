...
class OrderForm extends Form
{
    ...
    public $order_pesting_charge = 0;

    public $order_fitting_charge = 0;

    // public $totalTransportation = 0;
    public $order_transportation = 0;
    ...
    public function saveOrder(?Order $order = null): void
    {
        $this->validate();
        $orderValues = $this->getOrderValues();
        $order->update($orderValues);
        ...
        foreach ($this->selectedJobs as $job) {
            ...
            $orderJob = $order->orderJobs()->updateOrCreate(
                ['job_no' => $jobNo],
                $this->getOrderJobValues(array_merge($job, ['job_no' => $jobNo]))
            );
            if (isset($job['image']) && !is_string($job['image'])) {
                $this->syncMedia($orderJob, $job['image']);
            }
        }
        $order->orderJobs()->whereNotIn('job_no', $selectedJobNos)->delete();
    }
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'cancellation_reason' => ['nullable', 'string', 'max:255', Rule::when(fn () => $this->order_status == OrderStatusEnum::Cancelled->value, ['required'])],
        ];
        if ($this->isEdit) {
    ...
    public function calculateEstimate()
    {
        $estimate = (float) $this->job_total_amount
            + (float) $this->order_pesting_charge
            + (float) $this->order_fitting_charge
            + (float) $this->order_transportation
            - (float) $this->discount;
    ...
    protected function getOrderValues(): array
    {
        return [
            ...
            'pesting_charge' => $this->order_pesting_charge,
            'fitting_charge' => $this->order_fitting_charge,
            'transportation_charge' => $this->order_transportation,
    ...
    protected function getOrderJobValues(array $job): array
    {
        return [
    ...
    protected function syncMedia(OrderJob $job, $image): void
    {
        if ($image) {
            $job->addMedia($image)->toMediaCollection('order_job_image');
        }
    }
}
